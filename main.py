...
############################## PREPARE DATASET ##########################
train_data, test_data, user_num ,item_num, train_mat = fast_data_utils.load_all()

# construct the train and test datasets
train_dataset = fast_data_utils.FastBPRData(
                train_data, item_num, train_mat, args.num_ng, is_training=True, reuse_order=True)
test_dataset = fast_data_utils.FastBPRData(
                test_data, item_num, train_mat, 0, is_training=False, reuse_order=False)
########################### CREATE MODEL #################################
train_dataset.ng_sample()
model = SharpCF(user_num, item_num, args.factor_num, (args.free_szwin, len(train_dataset)), train_dataset.order)
model.cuda()
...
for epoch in range(args.epochs):
    ...
    for idx in range(num_batch):
        ...
        user, item_i, item_j = batch[:, 0], batch[:, 1], batch[:, 2]
    ...
    model.eval()
    logger.log("07.eval()")
    HR, NDCG = fast_evaluate.metrics(model, test_dataset, args.top_k, num_unit = args.test_num_ng + 1)
